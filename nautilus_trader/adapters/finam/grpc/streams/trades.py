...
"""
Trades stream manager for Finam gRPC API.
"""
...
class TradesStreamManager(BaseStreamManager):
    """
    Manages Trades streaming subscriptions for Finam gRPC API.

    Server Stream: SubscribeLatestTrades(symbol) -> stream of Trade updates

    Features:
    - Per-instrument Trades streams
    ...
    async def subscribe_trades(
        self,
        symbol: str,
        callback: Callable[[SubscribeLatestTradesResponse], None],
    ) -> None:
        """
        Subscribe to Trades updates for a symbol.

        Creates a server-side stream that sends real-time trade ticks.
        Each update contains a list of trades with price, size, side, and timestamp.
        ...
        callback : Callable[[SubscribeLatestTradesResponse], None]
            Handler for Trades updates
        """
        stream_id = f"trades_{symbol}"

        if stream_id in self._streams:
            self._logger.warning(f"Already subscribed to Trades for {symbol}")
            return
        ...
        async def stream_factory():
            ...
            return self._client.marketdata.SubscribeLatestTrades(
                request,
                metadata=metadata,
            )

        def message_handler(response: SubscribeLatestTradesResponse):
            """Handle incoming Trades messages."""
            try:
                callback(response)
            except Exception as e:
                self._logger.error(
                    f"Error in Trades callback for {symbol}: {e}",
                    exc_info=True,
                )
    ...
    async def unsubscribe_trades(self, symbol: str) -> None:
        """
        Unsubscribe from Trades updates for a symbol.
        ...
        self._logger.info(f"Unsubscribed from Trades: {symbol}")
